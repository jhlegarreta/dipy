...
def buan_bundle_profiles(
    ...
    *,
    ...
    mb = glob(Path(model_bundle_folder) / "*.trk")
    print(mb)

    mb.sort()

    bd = glob(Path(bundle_folder) / "*.trk")

    bd.sort()
    print(bd)
    org_bd = glob(Path(orig_bundle_folder) / "*.trk")
    org_bd.sort()
    print(org_bd)
    n = len(mb)
    ...
    for io in range(n):
        ...
        if len(orig_bundles) > 5:
            ...
            metric_files_names_dti = glob(Path(metric_folder) / "*.nii.gz")

            metric_files_names_csa = glob(Path(metric_folder) / "*.pam5")
